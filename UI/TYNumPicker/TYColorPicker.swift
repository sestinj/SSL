...
//  MYOA
//
//  Created by Nate Sesti on 5/26/19.
//  Copyright © 2019 Nate Sesti. All rights reserved.
//

import UIKit

protocol TYColorPickerDelegate {
    func selectedColor(_ color: UIColor)
}

class TYColorPicker: UIViewController {
    
    private enum ColorComponents: String {
        case r = "R"
        case g = "G"
        case b = "B"
    }
    
    private var currentComponent: ColorComponents = .r
    var delegate: TYColorPickerDelegate!
    var maxNumber: Int! = 255
    var bgGradients: [UIColor] = [.clear, .clear]
    ...
    private var selectedColor: UIColor = .white {
        didSet {
            self.numberLbl.text = "R:\(Int(selectedColor.r()*255)),G:\(Int(selectedColor.g()*255)),B:\(Int(selectedColor.b()*255))"
//            let _ = pickerView.applyGradient(colors: [selectedColor, selectedColor], type: .cross)
            pickerView.backgroundColor = selectedColor
            print(selectedColor)
        }
    }
    
    var defaultSelectedColor: UIColor = .white
    
    init(_ delegate: TYColorPickerDelegate) {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.01) {
            ...
            self.scrollToDefaultColor(self.defaultSelectedColor)
        }
    }
    ...
    private func scrollToDefaultColor(_ color: UIColor) {
        let offset = CGPoint(x: CGFloat(color.r()) * cellWidthIncludingSpacing - collectionView.contentInset.left, y: -collectionView.contentInset.top)
...
extension TYColorPicker: UICollectionViewDelegate, UICollectionViewDataSource {
...
extension TYColorPicker: UIScrollViewDelegate {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let selectedComponent = CGFloat(roundedIndex <= 0 ? 0 : Int(roundedIndex))/255.0
        switch currentComponent {
            case .r:
                selectedColor = selectedColor.withRComponent(selectedComponent)
            case .g:
                selectedColor = selectedColor.withGComponent(selectedComponent)
            case .b:
                selectedColor = selectedColor.withBComponent(selectedComponent)
        }
